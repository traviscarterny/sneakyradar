exports.handler = async function(event) {
  ...
  if (body.action === "trending") {
    ...
    try {
      ...
      // Fetch StockX + GOAT in parallel
      const [stockxRes, goatRes] = await Promise.all([
        fetch(`${KICKSDB_BASE}/stockx/products?query=${encodeURIComponent(q)}&limit=50&offset=${offset}&page=${page}`, { headers }).then(r => r.json()).catch(() => ({ data: [] })),
        fetch(`${KICKSDB_BASE}/goat/products?query=${encodeURIComponent(q)}&limit=50`, { headers }).then(r => r.json()).catch(() => ({ data: [] })),
      ]);

      let products = stockxRes?.data || [];
      const goatProducts = goatRes?.data || [];
      ...
      const goatMatches = products.filter(p => p._goat).length;
      const duration = Date.now() - startTime;
      console.log("KicksDB trending page", page, ":", products.length, "products, GOAT matched:", goatMatches, "in", duration, "ms");
